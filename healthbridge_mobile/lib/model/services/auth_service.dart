...
class AuthService {
  ...
  SessionModel? _tryDemoLogin({
    ...
    for (final account in _demoAccounts) {
      ...
      if (matchesUser && account.password == password) {
        return SessionModel(
          token:
              'demo-token-${account.user.role.toLowerCase()}-${account.user.id}',
          user: account.user,
        );
      }
    }
  ...
  bool _isDemoToken(String? token) =>
      token != null && token.startsWith('demo-token-');
}

class _DemoAuthAccount {
  const _DemoAuthAccount({required this.user, required this.password});
