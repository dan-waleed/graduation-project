...
import 'package:healthbridge_mobile/model/core/config/app_config.dart';
...
class AppDataService {
  ...
  Future<List<MedicationModel>> getMedications({
    ...
    if (_shouldUseDemoMode && !serverOnly) {
    ...
    try {
      ...
      if (_allowLocalDemoFallback && !serverOnly) {
        return List<MedicationModel>.from(_debugMedications);
      }
      rethrow;
    }
  ...
  Future<MedicationModel> ensureMedicationExists(
    ...
    if (_shouldUseDemoMode) {
      final existing = _debugMedications.cast<MedicationModel?>().firstWhere(
        (item) => item != null && _isSameMedication(item, medication),
        orElse: () => null,
      );
      if (existing != null) {
        return existing;
      ...
      _debugMedications.add(created);
      return created;
    }

    try {
      final response = await _apiClient.getList(
        'medications/?name=${Uri.encodeQueryComponent(medication.name)}',
      );
      ...
      // Try creating the medication directly if lookup fails.
    }
    ...
    return MedicationModel.fromJson(created);
  }
  ...
  bool _isSameMedication(MedicationModel left, MedicationModel right) {
    return left.name.trim().toLowerCase() == right.name.trim().toLowerCase() &&
        left.genericName.trim().toLowerCase() ==
            right.genericName.trim().toLowerCase() &&
        left.strength.trim().toLowerCase() ==
            right.strength.trim().toLowerCase();
  }
  ...
  String _resolveSubmittedPrescriptionStatus({
    required String requestedStatus,
    required bool requiresInsuranceApproval,
  }) {
    if (requestedStatus != 'Sent') {
      return requestedStatus;
    }
    return requiresInsuranceApproval ? 'PendingInsuranceApproval' : 'Approved';
  }

  void _ensureInsuranceOfficerApproval(
    ...
    if (nextStatus == 'Approved' &&
      ...
      throw const AppException(
        'هذه الوصفة تحتوي على دواء يحتاج موافقة مسبقة، ويجب اعتمادها من موظف التأمين أولًا.',
      );
    }
  }
  ...
  Future<PrescriptionModel> createPrescription({
    ...
    final resolvedStatus = _resolveSubmittedPrescriptionStatus(
    ...
    if (_shouldUseDemoMode) {
      ...
      if (status == 'Sent') {
        _upsertSubmittedDebugInsuranceRequest(prescription);
      }
      return prescription;
    }
  ...
  Future<PrescriptionModel> updatePrescriptionStatus({
    ...
    if (_shouldUseDemoMode && debugIndex != -1) {
      ...
      _ensureInsuranceOfficerApproval(current, resolvedStatus);
      ...
      if (status == 'Sent') {
        _upsertSubmittedDebugInsuranceRequest(updated);
      }
      return updated;
    }
    ...
    if (status == 'Sent' || status == 'Approved') {
      currentPrescription = await getPrescription(id);
      resolvedStatus = _resolveSubmittedPrescriptionStatus(
        requestedStatus: status,
        requiresInsuranceApproval:
            currentPrescription.requiresInsuranceApproval,
      );
      _ensureInsuranceOfficerApproval(currentPrescription, resolvedStatus);
    }

    final body = <String, dynamic>{'status': resolvedStatus};
  ...
  void _upsertSubmittedDebugInsuranceRequest(PrescriptionModel prescription) {
    final existingIndex = _debugInsuranceRequests.indexWhere(
      (item) => item.prescriptionId == prescription.id,
    );
    final requestStatus = prescription.status == 'PendingInsuranceApproval'
        ? 'Pending'
        : 'Approved';
    final responseNotes = requestStatus == 'Pending'
        ? ''
        : 'تمت الموافقة تلقائيًا من النظام.';
  ...
  Future<InsuranceRequestModel> createInsuranceRequest({
    ...
    if (_shouldUseDemoMode) {
      ...
      final existing = _debugInsuranceRequests
  ...
  Future<InsuranceRequestModel> updateInsuranceRequest({
    ...
    if (_shouldUseDemoMode && debugIndex != -1) {
      ...
      const allowedTransitions = <String, Set<String>>{
        'Pending': {'Approved', 'Rejected', 'NeedsUpdate'},
        'NeedsUpdate': {'Pending', 'Approved', 'Rejected'},
        'Approved': <String>{},
        'Rejected': <String>{},
      };
      if (status != current.status &&
          !(allowedTransitions[current.status]?.contains(status) ?? false)) {
        throw const AppException(
          'لا يمكن تغيير حالة الطلب بعد اعتماده أو رفضه.',
        );
      }
