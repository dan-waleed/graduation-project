...
import 'package:healthbridge_mobile/modelView/features/doctor/doctor_medication_add_view_model.dart';
...
class _DoctorPrescriptionCreateFormState
  ...
  Future<void> _savePrescription(String status) async {
    ...
    try {
      ...
      final createdPrescription = await service.createPrescription(
        ...
        SnackBar(
          content: Text(
            status == 'Draft'
                ...
                : createdPrescription.status == 'PendingInsuranceApproval'
                ? 'تم إرسال الوصفة لمراجعة موظف التأمين'
...
class DoctorMedicationAddScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (context) => DoctorMedicationAddViewModel(
        appRepository: context.read<AppRepository>(),
      )..initialize(),
      child: const _DoctorMedicationAddScreenView(),
    );
  }
}

class _DoctorMedicationAddScreenView extends StatelessWidget {
  const _DoctorMedicationAddScreenView();

  @override
  Widget build(BuildContext context) {
    final viewModel = context.watch<DoctorMedicationAddViewModel>();
    final filteredMedications = viewModel.filteredMedications;
    final selectedMedication = viewModel.selectedMedication;
    final coverageItem = viewModel.selectedCoverageItem;
    final fixedUsageCount = viewModel.fixedUsageCount;
    final fixedQuantity = viewModel.fixedQuantity;
    final fixedInstructions = viewModel.fixedInstructions;
    ...
    return HbScaffold(
      ...
      body: viewModel.isLoading
          ...
          : viewModel.availableMedications.isEmpty
          ? const HbEmptyState(
              title: 'لا توجد أدوية متاحة',
              message:
                  'يرجى إضافة الأدوية إلى قاعدة البيانات أو كتالوج التغطية حتى تظهر هنا.',
            )
                  ...
                  child: Column(
                    children: [
                      TextField(
                        ...
                        onChanged: viewModel.scheduleMedicationSearch,
                        onEditingComplete: () =>
                            viewModel.submitMedicationSearch(),
                        onSubmitted: viewModel.submitMedicationSearch,
                        onTapOutside: (_) => viewModel.submitMedicationSearch(),
                      ),
                      const SizedBox(height: 16),
                      const HbEmptyState(
                        title: 'لا توجد نتائج',
              ...
              children: [
                HbCustomCard(
                  child: Column(
                    children: [
                      ...
                      const SizedBox(height: 12),
                      DropdownButtonFormField<int>(
                        key: ValueKey(viewModel.selectedMedicationId),
                        initialValue: viewModel.selectedMedicationId,
                        isExpanded: true,
                        itemHeight: null,
                        decoration: const InputDecoration(
                          labelText: 'اختيار الدواء من قائمة',
                        ),
                        selectedItemBuilder: (context) => filteredMedications
                            .map(
                              (item) => Align(
                                alignment: AlignmentDirectional.centerStart,
                                child: Text(
                                  _medicationSelectedLabel(item),
                                ...
                                child: Padding(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      if (_medicationSecondaryLabel(
                                        item,
                                      ).isNotEmpty) ...[
                                        const SizedBox(height: 4),
                                        Text(
                                          _medicationSecondaryLabel(item),
                                          style: Theme.of(
                                            context,
                                          ).textTheme.bodySmall,
                                        ),
                        ...
                        onChanged: (value) {
                          ...
                          viewModel.selectMedication(value);
                        },
                      ),
                      const SizedBox(height: 12),
                      TextField(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                HbInfoRow(
                                  label: 'الحد الأعلى',
                                  value:
                                      '${coverageItem.maxQuantity} عبوة/وحدة',
                                ),
                                HbInfoRow(
                                  label: 'موافقة مسبقة',
                ...
                HbPrimaryButtonRow(
                  ...
                  onPrimaryPressed: () async {
                    try {
                      final resolvedMedication = await viewModel
                          .ensureSelectedMedicationExists();
                      ...
                      final selectedCoverageItem =
                          ...
                          context
                              .read<AppRepository>()
                              .findCoverageForMedication(resolvedMedication);
                      ...
                      final coveredAmount =
                          selectedCoverageItem?.coveredAmountFor(totalPrice) ??
                          0;
                      final employeeShare =
                      ...
                      Navigator.of(context).pop(
                        _DraftPrescriptionItem(
                          medicationId: resolvedMedication.id,
                          medicationName: resolvedMedication.name,
                          serviceName: resolvedMedication.name,
                          strength: resolvedMedication.strength,
                          dosageForm: resolvedMedication.dosageForm,
                          dosageInstructions:
                              '$fixedUsageCount • $fixedInstructions',
                          quantity: fixedQuantity,
                          duration:
                              viewModel.durationController.text.trim().isEmpty
                              ? 'غير محدد'
                              : viewModel.durationController.text.trim(),
                  ...
                  onSecondaryPressed: viewModel.clearDuration,
                ),
              ],
            ),
    );
...
String _medicationSelectedLabel(MedicationModel medication) {
  final genericName = medication.genericName.trim();
  final strength = medication.strength.trim();
  if (genericName.isNotEmpty && strength.isNotEmpty) {
    return '${medication.name} • $genericName • $strength';
  }
  if (genericName.isNotEmpty) {
    return '${medication.name} • $genericName';
  }
  if (strength.isNotEmpty) {
    return '${medication.name} • $strength';
  }
  return medication.name;
}

String _medicationSecondaryLabel(MedicationModel medication) {
  final parts = <String>[
    if (medication.genericName.trim().isNotEmpty) medication.genericName.trim(),
...
class _DoctorPrescriptionDetailScreenState
  ...
  Future<void> _submitPrescription(PrescriptionModel prescription) async {
    ...
    try {
      ...
      final updatedPrescription = await service.updatePrescriptionStatus(
        id: prescription.id,
        status: nextStatus,
      );
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            updatedPrescription.status == 'PendingInsuranceApproval'
                ? 'تم إرسال الوصفة لمراجعة موظف التأمين'
