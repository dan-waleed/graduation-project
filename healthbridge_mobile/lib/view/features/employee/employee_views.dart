...
import 'package:healthbridge_mobile/modelView/features/employee/employee_prescriptions_view_model.dart';
...
class EmployeeHomeScreen extends StatelessWidget {
  ...
  static const routePath = '/employee';
...
class EmployeePrescriptionsView extends StatelessWidget {
  const EmployeePrescriptionsView({super.key});

  static const routeName = 'employee-prescriptions';
  static const routePath = '/employee/prescriptions';

  @override
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (context) => EmployeePrescriptionsViewModel(
        appRepository: context.read<AppRepository>(),
      ),
      child: const _EmployeePrescriptionsViewBody(),
    );
  }
}

class _EmployeePrescriptionsViewBody extends StatelessWidget {
  const _EmployeePrescriptionsViewBody();
  ...
  @override
  Widget build(BuildContext context) {
    final viewModel = context.watch<EmployeePrescriptionsViewModel>();
    ...
    return HbScaffold(
      ...
      body: FutureBuilder<List<PrescriptionModel>>(
        ...
        builder: (context, snapshot) {
          ...
          final filteredPrescriptions = viewModel.filteredPrescriptions(
            prescriptions,
          );
                  ...
                  return Padding(
                    ...
                    child: _EmployeeRecordActionTile(
                      ...
                      onPressed: () async {
                        ...
                        unawaited(Future<void>.microtask(viewModel.refresh));
                      },
                    ),
                  );
                }),
...
class EmployeePrescriptionDetailView extends StatelessWidget {
  ...
  void _returnToPrescriptions(BuildContext context) {
    ...
    context.go(EmployeePrescriptionsView.routePath);
  }
                    ...
                    children: [
                      ...
                      HbCustomButton(
                        ...
                        onPressed: () => context.push(
                          '${EmployeeQrView.routePath}?id=${prescription.id}',
                        ),
...
class EmployeeMedicationHistoryView extends StatelessWidget {
  const EmployeeMedicationHistoryView({super.key});

  static const routeName = 'employee-medication-history';
  static const routePath = '/employee/history';
  ...
  Widget build(BuildContext context) {
    return HbScaffold(
      ...
      actions: _commonActions(context),
      body: FutureBuilder<List<PrescriptionModel>>(
        future: context.read<AppRepository>().getPrescriptions(),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const Center(child: CircularProgressIndicator());
          }
...
class _EmployeeRecordActionTile extends StatelessWidget {
  const _EmployeeRecordActionTile({
    required this.title,
...
class EmployeeDependentsView extends StatelessWidget {
  const EmployeeDependentsView({super.key});

  static const routeName = 'employee-dependents';
  static const routePath = '/employee/dependents';
  ...
  Widget build(BuildContext context) {
    return HbScaffold(
      ...
      body: FutureBuilder<List<DependentModel>>(
        future: context.read<AppRepository>().getDependents(),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const Center(child: CircularProgressIndicator());
          }
