  ...
  Widget build(BuildContext context) {
    return HbScaffold(
      ...
      body: FutureBuilder<List<EmployeeModel>>(
        future: context.read<AppDataService>().getEmployees(),
    ...
    return HbScaffold(
      ...
      body: patientId == null
          ...
          : FutureBuilder<List<dynamic>>(
              future: Future.wait([
                context.read<AppDataService>().getEmployee(patientId!),
                context.read<AppDataService>().getDependents(employeeId: patientId),
...
class _DoctorPrescriptionCreateFormState extends State<_DoctorPrescriptionCreateForm> {
  ...
  void initState() {
    ...
    _formSetupFuture = Future.wait([
      context.read<AppDataService>().getEmployees(),
      context.read<AppDataService>().getCoverageCatalog(),
    ]);
  ...
  Future<void> _loadDependentsForPatient(int? patientId) async {
    ...
    final dependents = await context.read<AppDataService>().getDependents(employeeId: patientId);
    if (!mounted) return;
    setState(() {
  ...
  Future<void> _savePrescription(String status) async {
    ...
    try {
      ...
      if (status == 'Sent' && prescription.requiresInsuranceApproval) {
        await service.createInsuranceRequest(prescriptionId: prescription.id);
      }
...
class _DoctorMedicationAddScreenState extends State<DoctorMedicationAddScreen> {
  ...
  Widget build(BuildContext context) {
    return HbScaffold(
      ...
      body: FutureBuilder<List<dynamic>>(
        future: Future.wait([
          context.read<AppDataService>().getMedications(),
          context.read<AppDataService>().getCoverageCatalog(category: 'Medication'),
        ]),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const Center(child: CircularProgressIndicator());
          ...
          _selectedMedication ??= medications.first;
          final selectedMedication = _selectedMedication;
          final coverageItem = selectedMedication == null
              ...
              : coverageCatalog.cast<CoverageCatalogItemModel?>().firstWhere(
                  ...
                  orElse: () => context.read<AppDataService>().findCoverageForMedication(selectedMedication),
                );
                ...
                child: Column(
                  children: [
                    DropdownButtonFormField<MedicationModel>(
                      ...
                      items: medications
                          .map(
                            (item) => DropdownMenuItem<MedicationModel>(
                              value: item,
                    ...
                    const SizedBox(height: 12),
                    TextField(
                      controller: TextEditingController(text: _selectedMedication?.name ?? ''),
                      readOnly: true,
                      decoration: const InputDecoration(labelText: 'اسم الدواء'),
                    ),
                    const SizedBox(height: 12),
                    TextField(
                      controller: TextEditingController(text: _selectedMedication?.genericName ?? ''),
                      readOnly: true,
                      decoration: const InputDecoration(labelText: 'الاسم العلمي'),
                    ),
                    const SizedBox(height: 12),
                    TextField(
                      controller: TextEditingController(text: _selectedMedication?.strength ?? ''),
                      readOnly: true,
                      decoration: const InputDecoration(labelText: 'التركيز'),
                    ),
                    const SizedBox(height: 12),
                    TextField(
                      controller: TextEditingController(text: _selectedMedication?.dosageForm ?? ''),
                      readOnly: true,
                    ...
                    HbCustomInput(
                      ...
                      label: 'التعليمات',
                      hint: 'مثال: بعد الطعام مع كمية كافية من الماء',
                      maxLines: 3,
                    ),
                    const SizedBox(height: 12),
              ...
              HbPrimaryButtonRow(
                ...
                onPrimaryPressed: () {
                  ...
                  final selectedCoverageItem = coverageItem ??
                      context.read<AppDataService>().findCoverageForMedication(medication);
                  final requestedUnits = _firstNumberInText(_quantityController.text, fallback: 1);
...
class _DoctorPrescriptionDetailScreenState extends State<DoctorPrescriptionDetailScreen> {
  ...
  Future<void> _submitPrescription(PrescriptionModel prescription) async {
    ...
    final service = context.read<AppDataService>();
    try {
      final nextStatus = prescription.requiresInsuranceApproval
      ...
      if (prescription.requiresInsuranceApproval) {
        await service.createInsuranceRequest(
        ...
        SnackBar(
          content: Text(
            prescription.requiresInsuranceApproval
                ? 'تم إرسال الوصفة لمراجعة التأمين بنجاح'
    ...
    return HbScaffold(
      ...
      body: widget.prescriptionId == null
          ...
          : FutureBuilder<PrescriptionModel>(
              future: context.read<AppDataService>().getPrescription(widget.prescriptionId!),
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return const Center(child: CircularProgressIndicator());
                }
