...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Future<List<NotificationModel>> _loadNotifications() {
    return _showUnreadOnly
        ? context.read<AppRepository>().getUnreadNotifications()
        : context.read<AppRepository>().getNotifications();
  }
  ...
  Future<void> _markAllRead() async {
    ...
    final notificationCenter = context.read<NotificationCenterViewModel>();
    if (mounted) {
      setState(() {
  ...
  Future<void> _markNotificationRead(int notificationId) async {
    ...
    try {
      ...
      await context.read<NotificationCenterViewModel>().refreshUnreadCount();
      rethrow;
    }
  }
  ...
  Future<void> _navigateToRelatedEntity(NotificationModel item) async {
    final appDataService = context.read<AppRepository>();
    final authController = context.read<AuthViewModel>();
    ...
    if (item.relatedEntityType == 'Prescription' && relatedId != null) {
      switch (currentRole) {
        ...
        default:
          context.push('${EmployeePrescriptionDetailView.routePath}?id=$relatedId');
      }
      return;
    }
    if (item.relatedEntityType == 'InsuranceRequest' && relatedId != null) {
      ...
      if (match != null) {
        switch (currentRole) {
          ...
          default:
            context.push('${EmployeePrescriptionDetailView.routePath}?id=${match.prescriptionId}');
        }
        return;
      }
    }
